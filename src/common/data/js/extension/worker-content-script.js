...
const ClassWorkerContentScript = (base, browser, setTimeout) => {
	...
	const creditCardNameFields = new Set(["cc-name"]);
	const creditCardNumberFields = new Set(["cc-number"]);
	const creditCardCSCFields = new Set(["cc-csc"]);
	const creditCardExpiryDateFields = new Set(["cc-exp"]);
	const creditCardExpiryDateMonthFields = new Set(["cc-exp-month"]);
	const creditCardExpiryDateYearFields = new Set(["cc-exp-year"]);
	...
	const newPasswordFields = new Set(["new-password"]);
	const currentPasswordFields = new Set(["current-password"]);
	const totpFields = new Set(["one-time-code"]);
			...
			if (e.ctrlKey && e.shiftKey && e.code === "KeyL") {
				...
				base.emit(
					...
					async (response) => {
						fillAll = true;
						requestSecret(
							response.data[nextFillAllIndex % response.data.length].secret_id,
						);
		...
		styleProtectionObserver = new MutationObserver((mutations) => {
			...
			mutations.forEach((mutation) => {
				if (
					...
					if (
						(protectedElements.has(target) || isProtectedElement(target)) &&
	...
	function isProtectedElement(element) {
		return (
			element.classList.contains("psono-drop") ||
	...
	function restoreElementStyles(element) {
		...
		if (originalStyleData) {
			...
			const legitimateRuntimeClasses = currentClasses.filter((cls) =>
	...
	function removeNonLegitimateClasses(element) {
		...
		const currentClasses = Array.from(element.classList);
		const legitimateClasses = currentClasses.filter((cls) =>
			allowedClasses.has(cls),
		);
	...
	function checkAndRestoreParentVisibility(element) {
		...
		if (isBodyOrHtml) {
			...
			if (opacity < 0.1) {
				...
				console.warn(
					"PSONO Security: Restored parent element opacity from",
					opacity,
					"to 1",
				);
			}
			...
			if (computedStyle.visibility === "hidden") {
				...
				console.warn(
					"PSONO Security: Restored parent element visibility from hidden to visible",
				);
			}
			...
			if (computedStyle.display === "none") {
				element.style.setProperty("display", "block", "important");
				console.warn(
					"PSONO Security: Restored parent element display from none to block",
				);
			}
		}
	...
	function formSubmitCatcher(form) {
		const passwordFields = querySelectorAllIncShadowRoots(
			form,
			"input[type='password']",
		);
		...
		const formSubmitEventListener = function (event) {
			...
			const formData = getUsernameAndPassword(form);
			if (formData) {
		...
		let submitButtons = querySelectorAllIncShadowRoots(
			form,
		...
		if (submitButtons.length < 1) {
			...
			const otherButtons = querySelectorAllIncShadowRoots(
				form,
				"button[type='button'], input[type='button'], input[type='image'], button:not([type]), a, span",
			);
			submitButtons = [];
			for (let i = 0; i < otherButtons.length; i++) {
				const buttonContent =
					otherButtons[i].tagName.toLowerCase() === "input"
						? otherButtons[i].value
						: otherButtons[i].innerText;

				if (
					passwordSubmitButtonLabels.has(buttonContent.toLowerCase().trim())
				) {
		...
		for (let i = 0; i < submitButtons.length; i++) {
			submitButtons[0].addEventListener(
				"click",
	...
	function getUsernameAndPassword(form) {
		const fields = querySelectorAllIncShadowRoots(
			form,
		...
		node.querySelectorAll("*").forEach((el) => {
			if (el.shadowRoot) {
				inputs = inputs.concat(
					querySelectorAllIncShadowRoots(el.shadowRoot, selector),
				);
			}
		});
	...
	function addPasswordFormButtons(document) {
		...
		const inputs = querySelectorAllIncShadowRoots(
			document,
		...
		for (let i = 0; i < inputs.length; ++i) {
			...
			if (
				inputs[i].classList.contains("psono-addPasswordFormButtons-covered")
			) {
				continue;
			}
			...
			for (let r = i - 1; r > -1; r--) {
				...
				if (
					Object.hasOwn(inputs[i], "checkVisibility") &&
					inputs[i].checkVisibility() &&
					inputs[r].offsetWidth < 90
				)
					continue; // we don't modify input fields that are too small if they are visible

				// username field is inputs[r]
				paddingRight = window
					.getComputedStyle(inputs[r])
	...
	function addUsernameFormButtons(document) {
		...
		for (let i = 0; i < inputs.length; ++i) {
			if (
				inputs[i].classList.contains("psono-addUsernameFormButtons-covered")
			) {
				continue;
			}
	...
	function findCreditCardInputFields(document) {
		...
		for (let i = 0; i < inputs.length; ++i) {
			if (
				inputs[i].classList.contains("psono-findCreditCardInputFields-covered")
			) {
				continue;
			}

			if (
				creditCardAllFields.has(inputs[i].autocomplete.trim().toLowerCase())
			) {
	...
	function findIdentityInputFields(document) {
		...
		for (let i = 0; i < inputs.length; ++i) {
			if (
				inputs[i].classList.contains("psono-findIdentityInputFields-covered")
			) {
				continue;
			}
	...
	async function click(evt, target, document, input) {
		if (Object.hasOwn(target, "checkVisibility") && !target.checkVisibility()) {
		...
		if (!securityCheck.safe) {
			console.warn(
				"PSONO Security: Dropdown blocked due to security threats:",
			...
			if (securityCheck.threats.includes("existing_popovers")) {
				console.warn(
					"PSONO Security: Blocking dropdown due to existing popover elements",
				);
				return;
			}
		}
		...
		base.emit(
			...
			document.location.toString(),
			async (response) => {
				const openDatastoreClass = "psono_open-datastore-" + uuid.v4();
				const generatePasswordClass = "psono_generate-password-" + uuid.v4();
					...
					for (let i = 0; i < response.data.length; i++) {
						...
						if (
							...
							!response.data[i].name
								.toLowerCase()
								.includes(response.data[i].description.toLowerCase())
						) {
							sanitizedDescription = sanitizeText(
								" (" + response.data[i].description + ")",
							);
						}
						if (sanitizedDescription) {
							sanitizedDescription =
								'<span style="font-size: 11px !important; cursor: pointer !important;">' +
								sanitizedDescription +
								"</span>";
						}
						const requestSecretClass = "psono_request-secret-" + uuid.v4();
						let style = "";
						if (i >= 5) {
							style = "display:none !important";
						}
						dropcontent +=
							'<li class="psono_request-secret" style="' +
							style +
							'"><div class="' +
							requestSecretClass +
				...
				setTimeout(() => {
					const shadowRoot = dropInstance.getShadowRoot();

					const openDatastoreElements = shadowRoot.querySelectorAll(
						`.${openDatastoreClass}`,
					);
					for (const el of openDatastoreElements) {
					...
					const generatePasswordElements = shadowRoot.querySelectorAll(
						`.${generatePasswordClass}`,
					);
					for (const el of generatePasswordElements) {
						el.addEventListener("click", (event) => {
							event.stopPropagation();
							generate_password();
						});
					...
					for (let i = 0; i < requestSecretClasses.length; i++) {
						const className = requestSecretClasses[i]["class"];
						const secretId = requestSecretClasses[i]["secret_id"];
						const secretElements = shadowRoot.querySelectorAll(`.${className}`);
					...
					if (response.data.length > 5) {
						const searchInput = shadowRoot.querySelector(".psono-search-input");
						const listItems = shadowRoot.querySelectorAll(
							".psono_request-secret",
						);
						...
						if (searchInput) {
							...
							searchInput.addEventListener("keyup", () => {
								const searchStrings = searchInput.value
									.toLowerCase()
									.split(" ");
								...
								function match(searched) {
									...
									for (let ii = searchStrings.length - 1; ii >= 0; ii--) {
										if (searched.indexOf(searchStrings[ii]) > -1) {
											containCounter++;
										}
									}
		...
		for (const point of testPoints) {
			...
			if (elements.length > 0) {
				...
				for (let i = 0; i < elements.length; i++) {
					...
					if (
						["LABEL", "FORM", "INPUT"].includes(element.tagName) &&
						!element.id.startsWith("test-") &&
					...
					if (isSuspiciousElement(element)) {
						...
						if (overlaps) {
							console.warn(
								"PSONO Security: Overlay conflict detected at point",
								point,
							);
							...
							console.warn(
								"PSONO Security: Attempting to lower z-index from",
								currentComputedStyle.zIndex,
								"to 2147483630",
							);
							...
							if (isSuspiciousElement(element)) {
								console.error(
									"PSONO Security: Z-index lowering failed, element still interfering",
								);
								return true;
							} else {
								console.warn(
									"PSONO Security: Z-index lowering successful, continuing",
								);
	...
	function performSecurityCheck() {
		...
		if (existingPopovers.length > 0) {
			...
			console.warn(
				"PSONO Security: Existing popover elements detected:",
				existingPopovers,
			);
		}
		...
		if (highZElements.length > 0) {
			...
			console.warn(
				"PSONO Security: High z-index elements detected:",
				highZElements,
			);
		}
	...
	function createDropdownMenu(setup_event, content, document) {
		...
		const onDocumentClick = (event) => {
			...
			if (!isClickInsideDropdown) {
				...
				const shadowDropdowns = document.querySelectorAll(
					'[id^="psono_drop-"]',
				);
		...
		function open() {
			...
			setTimeout(() => {
				...
				if (overlayDetected) {
					console.error(
						"PSONO Security: Overlay attack detected - closing dropdown for safety",
					);
					close();
					return;
				}
		...
		function manualOverlayCheck() {
			...
			if (overlayDetected) {
				console.error(
					"PSONO Security: Manual check detected overlay attack - closing dropdown",
				);
				close();
				return true;
			}
	...
	function findFieldByName(fieldName) {
		...
		return Array.from(elements).find((element) => {
			if (
				(element.id && element.id.toLowerCase() === searchName) ||
				(element.name && element.name.toLowerCase() === searchName) ||
				(element.getAttribute("aria-label") &&
					element.getAttribute("aria-label").toLowerCase() === searchName) ||
				(element.placeholder &&
					element.placeholder.toLowerCase() === searchName)
			) {
				return true;
			}
			...
			if (element.id) {
				...
				if (
					labelFor &&
					labelFor.textContent.toLowerCase().trim() === searchName
				) {
					return true;
				}
			}
			...
			while (parent) {
				if (
					...
					parent.textContent.toLowerCase().trim() === searchName
				) {
					return true;
				}
	...
	function onFillPassword(data, sender, sendResponse) {
		...
		for (let i = 0; i < myForms.length; i++) {
			...
			if (Object.hasOwn(data, "password") && data.password !== "") {
				foundPassword = true;
			...
			if (
				...
				Object.hasOwn(data, "submit") &&
				data.submit && //https website
				Object.hasOwn(data, "auto_submit") &&
		...
		for (let i = 0; i < identityInputFields.length; i++) {
			if (
				usernameFields.has(
					identityInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
		...
		if (!foundUsername) {
			...
			const inputs = Array.from(
				querySelectorAllIncShadowRoots(document, "input"),
			);

			let potentialUsernameFields = inputs.filter(
				(input) => input.name.toLowerCase() === "username",
			);
			if (potentialUsernameFields.length === 0) {
				potentialUsernameFields = inputs.filter(
					(input) => input.name.toLowerCase() === "email",
				);
			}
		...
		if (Object.hasOwn(data, "custom_fields") && data.custom_fields) {
		...
		for (let i = 0; i < creditCardInputFields.length; i++) {
			if (
				creditCardNameFields.has(
					creditCardInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
				Object.hasOwn(data, "credit_card_name") &&
				data.credit_card_name !== ""
			) {
				fillFieldHelper(creditCardInputFields[i], data.credit_card_name);
			}
			if (
				creditCardNumberFields.has(
					creditCardInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
				Object.hasOwn(data, "credit_card_number") &&
				data.credit_card_number !== ""
			) {
				fillFieldHelper(creditCardInputFields[i], data.credit_card_number);
			}
			if (
				creditCardCSCFields.has(
					creditCardInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
				Object.hasOwn(data, "credit_card_cvc") &&
				data.credit_card_cvc !== ""
			) {
				fillFieldHelper(creditCardInputFields[i], data.credit_card_cvc);
			}
			if (
				creditCardExpiryDateFields.has(
					creditCardInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
				Object.hasOwn(data, "credit_card_valid_through") &&
				data.credit_card_valid_through !== ""
			) {
				fillFieldHelper(
					creditCardInputFields[i],
					data.credit_card_valid_through,
				);
			}
			if (
				creditCardExpiryDateMonthFields.has(
					creditCardInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
				Object.hasOwn(data, "credit_card_valid_through") &&
				data.credit_card_valid_through !== ""
				...
				fillFieldHelper(
					creditCardInputFields[i],
					data.credit_card_valid_through.slice(0, 2),
				);
			}
			if (
				creditCardExpiryDateYearFields.has(
					creditCardInputFields[i].autocomplete.trim().toLowerCase(),
				) &&
				Object.hasOwn(data, "credit_card_valid_through") &&
				data.credit_card_valid_through !== ""
				...
				fillFieldHelper(
					creditCardInputFields[i],
					data.credit_card_valid_through.slice(2, 4),
				);
			}
		}
	}
	...
	function onFillIdentity(data, sender, sendResponse) {
		for (let i = 0; i < identityInputFields.length; i++) {
			...
			if (
				...
				((Object.hasOwn(data, "identity_first_name") &&
					data.identity_first_name !== "") ||
					(Object.hasOwn(data, "identity_last_name") &&
						data.identity_last_name !== ""))
			) {
				const fullName = [
					Object.hasOwn(data, "identity_first_name")
						? data.identity_first_name
				...
				Object.hasOwn(data, "identity_address") &&
				...
				(autocompleteValue === "address-line1" ||
					...
					autocompleteValue === "address-line3") &&
				Object.hasOwn(data, "identity_address") &&
				data.identity_address !== ""
			) {
				fillFieldHelper(identityInputFields[i], data.identity_address);
				...
				Object.hasOwn(data, "identity_phone_number") &&
				data.identity_phone_number !== ""
			) {
				fillFieldHelper(identityInputFields[i], data.identity_phone_number);
			} else if (
				autocompleteValue === "email" &&
				Object.hasOwn(data, "identity_email") &&
				data.identity_email !== ""
			) {
				fillFieldHelper(identityInputFields[i], data.identity_email);
			} else if (
				autocompleteValue === "organization" &&
				Object.hasOwn(data, "identity_company") &&
				data.identity_company !== ""
			) {
		...
		inputs.forEach((input) => {
			...
			Object.entries(fieldNameMappings).forEach(([fieldName, dataProperty]) => {
				if (inputName === fieldName || inputName.includes(fieldName)) {
					if (Object.hasOwn(data, dataProperty) && data[dataProperty] !== "") {
	 ...
	 */
	const getSearchWebsitePasswordsByUrlfilter = (url) => {
	...
	async function onReturnSecret(data, sender, sendResponse) {
		...
		function autofill() {
			for (let i = 0; i < myForms.length; i++) {
				if (
					(myForms[i].username &&
						myForms[i].username.isEqualNode(lastRequestElement)) ||
					(myForms[i].password &&
						myForms[i].password.isEqualNode(lastRequestElement)) ||
					fillAll
				) {
		...
		if (isIframe) {
			const filter = getSearchWebsitePasswordsByUrlfilter(
