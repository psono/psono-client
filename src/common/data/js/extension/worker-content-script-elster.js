...
var ClassWorkerContentScriptElster = (base, browser, setTimeout) => {
	let lastCloseTime = 0;
	...
	function activate() {
		if (
			...
			document.defaultView.location.origin +
				document.defaultView.location.pathname ===
				"https://www.elster.de/bportal/login/softpse"
		) {
	...
	function fillCertificate(data) {
		...
		["keydown", "keypress", "keyup", "change", "blur", "input"].forEach(
			(eventType) => {
	...
	function addDropdownMenuButton(document) {
		...
		const elsterDropdownMenu = document.getElementById(
			"psonoDropdownMenuElster",
		); // check if we already did run our logic
	...
	function handleButtonClick(evt, target, document) {
		base.emit(
			...
			async (response) => {
				...
				setTimeout(() => {
					const element = dropInstance.getElement();

					const openDatastoreElements =
						element.getElementsByClassName(openDatastoreClass);
					...
					for (let i = 0; i < requestSecretClasses.length; i++) {
						...
						const secretElements = element.getElementsByClassName(className);
					...
					if (response.data.length > 5) {
						const searchInput = document.querySelector(".psono-search-input");
