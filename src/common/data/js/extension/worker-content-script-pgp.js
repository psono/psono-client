...
var ClassWorkerContentScriptPGP = (base, browser, setTimeout) => {
	...
	const supportedHoster = [
		...
		},
		{
			name: "livecom",
			domain_filter: () =>
				window.location.host.toLowerCase() === "outlook.live.com",
			getPgpContent: (node) => {
		...
		},
		{
			...
			domain_filter: () =>
				window.location.host.toLowerCase() === "outlook.office365.com",
			getPgpContent: (node) => {
				const directParent = node.parentNode;
				return directParent.textContent;
			},
		...
		},
		{
			name: "yahoo",
			domain_filter: () =>
				window.location.host.toLowerCase() === "mail.yahoo.com",
			getPgpContent: (node) => {
				const directParent = node.parentNode;
				return directParent.textContent;
			},
	...
	function get_supportedHoster(type) {
		for (let i = 0; i < supportedHoster.length; i++) {
			if (
				Object.hasOwn(supportedHoster[i], "domain_filter") &&
				supportedHoster[i].domain_filter()
			) {
				...
				acceptNode: (node) => {
					if (
						...
						!node.parentNode.classList.contains(
							"psono-add_pgp_message_readers-covered",
						) &&
			...
			if (parent) {
				...
				if (imgElement) {
					imgElement.addEventListener("click", function () {
						decryptGPG(
							base64ToString(this.getAttribute("data-pgp-content")),
	...
	function add_pgp_message_writers(document) {
		...
		for (let i = 0; i < fields.length; ++i) {
			if (
				fields[i].classList.contains("psono-add_pgp_message_writers-covered")
			) {
				continue;
			}
