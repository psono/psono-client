...
function checkCertificateExpiration(certPath) {
	try {
		...
		const result = execSync(`certutil -dump "${certPath}"`, {
			encoding: "utf8",
		});
		...
		if (notAfterMatch) {
			...
			if (expirationDate < now) {
				...
				console.error(
					`Certificate expired ${Math.floor((now - expirationDate) / (1000 * 60 * 60 * 24))} days ago`,
				);
				process.exit(1);
			} else {
				...
				if (daysUntilExpiry < 30) {
					console.warn(
						`⚠️  WARNING: Certificate expires in ${daysUntilExpiry} days - consider renewing soon`,
					);
				}
			}
		} else {
			console.warn(
				"Could not parse certificate expiration date from certutil output",
			);
		}
	} catch (error) {
...
function main() {
	if (!process.env.WINDOWS_SIGNING_CERTIFICATE) {
		console.log(
			"No Windows signing certificate provided via WINDOWS_SIGNING_CERTIFICATE environment variable",
		);
		return;
	}
	...
	const imgBuffer = Buffer.from(
		process.env.WINDOWS_SIGNING_CERTIFICATE,
		"base64",
	);
