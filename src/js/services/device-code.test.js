...
describe("Service: device-code test suite", () => {
	...
	it("claimDeviceCode successful claim", async () => {
		...
		const result = await deviceCodeService.claimDeviceCode(
			"device_code_id",
			"secret_box_key",
		);
	...
	test.each([
		{
			...
			expectedLogMessage:
				"User secrets not available in store for device code claim.",
		},
		{
		...
		},
		{
			...
			expectedLogMessage:
				"Authentication token or session secret key not available for device code claim.",
		},
		{
	...
	])("claimDeviceCode fails with $description", async ({
		nullField,
		expectedError,
		expectedLogMessage,
	}) => {
	...
	it("claimDeviceCode fails with encryption error", async () => {
		const mockState = {
			user: {
				userPrivateKey: "mock_private_key",
				userSecretKey: "mock_secret_key",
		...
		expect(consoleErrorSpy).toHaveBeenCalledWith(
			"Error encrypting credentials bundle for device code claim:",
			encryptionError,
		);
	...
	it("claimDeviceCode fails with API client error", async () => {
		const mockState = {
			user: {
				userPrivateKey: "mock_private_key",
				userSecretKey: "mock_secret_key",
