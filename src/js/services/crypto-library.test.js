...
describe("Service: cryptoLibraryService test suite #1", () => {
	it("cryptoLibraryService exists", () => {
		expect(cryptoLibraryService).toBeDefined();
	});
});

describe("Service: cryptoLibraryService test suite #2", () => {
	it("cryptoLibraryService exists", () => {
		expect(cryptoLibraryService).toBeDefined();
	});

	it("cryptoLibraryService randomBytes", () => {
		expect(cryptoLibraryService.randomBytes(16).length).toBe(16);
		expect(cryptoLibraryService.randomBytes(32).length).toBe(32);
		expect(cryptoLibraryService.randomBytes(64).length).toBe(64);
	});
    ...
    */
	...
	it("generateAuthkey works", async () => {
		jest.useFakeTimers();
		jest.spyOn(global, "setTimeout");

		let result = await cryptoLibraryService.generateAuthkey(
			"test@example.com",
			"123456",
		);
		...
		result = await cryptoLibraryService.generateAuthkey(
			"test2@example.com",
			"1234567",
		);
	...
	it("generate_secret_key returns a 32 bytes long key", () => {
		let bytes;
		bytes = 32;
		return expect(
	...
	it("sha1 abc", () => {
		const abc = cryptoLibraryService.sha1("abc");
		return expect(abc).toBe("a9993e364706816aba3e25717850c26c9cd0d89d");
	});

	it("sha256 abc", () => {
		const abc = cryptoLibraryService.sha256("abc");
		return expect(abc).toBe(
			"ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad",
		);
	});

	it("sha512 abc", () => {
		const abc = cryptoLibraryService.sha512("abc");
		return expect(abc).toBe(
			"ddaf35a193617abacc417349ae20413112e6fa4e89a97ea20a9eeee64b55d39a2192992a274fc1a836ba3c23a3feebbd454d4423643ce80e2a9ac94fa54ca49f",
		);
	});

	it("sha1 empty string", () => {
		const empty = cryptoLibraryService.sha1("");
		return expect(empty).toBe("da39a3ee5e6b4b0d3255bfef95601890afd80709");
	});

	it("sha256 empty string", () => {
		const empty = cryptoLibraryService.sha256("");
		return expect(empty).toBe(
			"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
		);
	});

	it("sha512 empty string", () => {
		const empty = cryptoLibraryService.sha512("");
		return expect(empty).toBe(
			"cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e",
		);
	});

	it("generatePublicPrivateKeypair returns a pair of 32 bytes long keys", () => {
		let bytes, pair;
		bytes = 32;
	...
	it("generatePublicPrivateKeypair returned pairs are different", () => {
		let pair1, pair2;
	...
	it("generateUserSauce", () => {
		let bytes, user_sauce1, user_sauce2;
		bytes = 32;
		user_sauce1 = cryptoLibraryService.generateUserSauce();
		user_sauce2 = cryptoLibraryService.generateUserSauce();
		expect(converterService.fromHex(user_sauce1).length).toBe(bytes);
		return expect(user_sauce1).not.toBe(user_sauce2);
	});

	it("getChecksum", () =>
		expect(cryptoLibraryService.getChecksum("RBLbEDsvgnU", 2)).toBe("4A"));

	it("generateRecoveryCode", () => {
		const recovery_code = cryptoLibraryService.generateRecoveryCode();

		expect(recovery_code.bytes.length).toBe(16);
		expect(recovery_code.hex).toBe(converterService.toHex(recovery_code.bytes));
		expect(recovery_code.hex).toBe(
			converterService.wordsToHex(recovery_code.words),
		);
		expect(recovery_code.hex).toBe(
			converterService.base58ToHex(recovery_code.base58),
		);

		const chunks = helperService.splitStringInChunks(recovery_code.base58, 11);

		for (let i = 0; i < chunks.length; i++) {
			chunks[i] += cryptoLibraryService.getChecksum(chunks[i], 2);
		}

		return expect(recovery_code.base58_checksums).toBe(chunks.join(""));
	});

	it("recoveryCodeStripChecksums", () => {
		const stripped = cryptoLibraryService.recoveryCodeStripChecksums(
			"UaKSKNNixJY2ARqGDKXduo4c2N",
		);
		return expect(stripped).toBe("UaKSKNNixJYRqGDKXduo4c");
	});

	it("recoveryCodeStripChecksums chunk < 2", () => {
		try {
			cryptoLibraryService.recoveryCodeStripChecksums("A");
		} catch (err) {
	...
	it("recoveryPasswordChunkPassChecksum positive", () =>
		expect(
			cryptoLibraryService.recoveryPasswordChunkPassChecksum("UaKSKNNixJY2A"),
		).toBeTruthy());

	it("recoveryPasswordChunkPassChecksum negative", () =>
		expect(
			cryptoLibraryService.recoveryPasswordChunkPassChecksum("UaKSKNNixJY2B"),
		).toBeFalsy());

	it("decryptSecret", async () => {
		jest.useFakeTimers();
		jest.spyOn(global, "setTimeout");
		...
		const result = await cryptoLibraryService.decryptSecret(
			text,
			nonce,
			password,
			user_sauce,
	...
	it("encryptSecret", () => {
		...
		expect(
			converterService.fromHex(encrypted_data.text).length,
		).toBeGreaterThan(0);
	...
	it("decryptData works", () => {
		...
		return expect(
			cryptoLibraryService.decryptData(text, nonce, secret_key),
		).toBe(data);
	});

	it("encryptData works", () => {
	...
	it("nacl's signing.verify works", () => {
