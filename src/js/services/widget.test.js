...
describe("Service: widgetService - moveItem test suite", () => {
	...
	beforeEach(() => {
		...
		getPasswordDatastoreSpy = jest.spyOn(
			datastorePasswordService,
			"getPasswordDatastore",
		);
		handleDatastoreContentChangedSpy = jest
		...
		datastoreSaveDatastoreContentSpy = jest
			.spyOn(datastoreService, "saveDatastoreContent")
			.mockResolvedValue();
		saveUserDatastoreContentSpy = jest
			.spyOn(datastoreUserService, "saveDatastoreContent")
			.mockResolvedValue();
	...
	describe("moveItem - basic functionality", () => {
		...
		it("should handle moving an item to a new folder in password datastore", async () => {
			...
			const [type, description, savedDatastore] =
				datastoreSaveDatastoreContentSpy.mock.calls[0];
	...
	describe("moveItem - permission checks", () => {
		it("should prevent moving item without write rights on target", async () => {
			...
			expect(global.alert).toHaveBeenCalledWith(
				"Sorry, but you don't have write rights on target",
			);
			expect(datastoreSaveDatastoreContentSpy).not.toHaveBeenCalled();
			expect(writeShareSpy).not.toHaveBeenCalled();
		});
	...
	describe("moveItem - shared folder operations with share content verification", () => {
		...
		it("should save correct content when moving item between shared folders", async () => {
			...
			const [shareId1, shareData1, shareSecretKey1] =
				writeShareSpy.mock.calls[0];
			...
			const [shareId2, shareData2, shareSecretKey2] =
				writeShareSpy.mock.calls[1];
	...
	describe("moveItem - share rights inheritance", () => {
		it("should update datastore with inherited share_rights when moving item to folder with share", async () => {
			...
			const updateParentsSpy = jest
				.spyOn(datastorePasswordService, "updateParents")
				.mockImplementation(() => {});
			getPasswordDatastoreSpy.mockResolvedValue(mockDatastore);
