...
function rebuildMockMap() {
	mockDomainSynonymMap = helperService.buildDomainSynonymMap(
		mockServerSynonyms,
		mockCustomSynonyms,
	);
}
...
describe("Service: domain-synonyms test suite", () => {
	beforeEach(() => {
		mockServerSynonyms = [];
		mockCustomSynonyms = [];
		rebuildMockMap();
	...
	it("domain-synonyms service exists", () => {
		expect(domainSynonymsService).toBeDefined();
	});

	it("getHardcodedSynonyms returns hardcoded groups", () => {
	...
	describe("getSynonymsForDomain", () => {
		it("returns synonyms for microsoft.com", () => {
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("microsoft.com");
		...
		it("returns synonyms for google.com", () => {
		...
		it("returns empty array for non-synonym domain", () => {
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("example.com");
		...
		it("includes custom synonyms when set", () => {
			setMockCustomSynonyms([
				["mycompany.com", "mycompany.net", "mycompany.org"],
			]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("mycompany.com");
			expect(synonyms).toContain("mycompany.net");
			expect(synonyms).toContain("mycompany.org");
		});
	});

	describe("expandUrlFilterWithSynonyms", () => {
		it("expands single domain with synonyms", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("microsoft.com");
		...
		it("keeps non-synonym domains as-is", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("example.com");
		...
		it("handles null/undefined input", () => {
			...
			const expanded2 =
				domainSynonymsService.expandUrlFilterWithSynonyms(undefined);
			expect(expanded2).toBe("");
		});

		it("expands with wildcard prefix (synonyms added as-is)", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("*.microsoft.com");
			expect(expanded).toContain("*.microsoft.com");
			expect(expanded).toContain("live.com");
		...
		it("does not expand with port suffix (specific port blocks synonym expansion)", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("microsoft.com:8080");
		...
		it("expands with wildcard port (synonyms added as-is)", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("microsoft.com:*");
			expect(expanded).toContain("microsoft.com:*");
			expect(expanded).toContain("live.com");
		...
		it("does not expand with both wildcard and specific port", () => {
			const expanded = domainSynonymsService.expandUrlFilterWithSynonyms(
				"*.microsoft.com:8080",
			);
			expect(expanded).toContain("*.microsoft.com:8080");
		...
		it("handles multiple domains in input (space-separated)", () => {
			const expanded = domainSynonymsService.expandUrlFilterWithSynonyms(
		...
		it("handles multiple domains in input (comma-separated)", () => {
			const expanded = domainSynonymsService.expandUrlFilterWithSynonyms(
				"microsoft.com,example.com",
			);
			expect(expanded).toContain("microsoft.com");
		...
		it("handles multiple domains in input (semicolon-separated)", () => {
			const expanded = domainSynonymsService.expandUrlFilterWithSynonyms(
				"microsoft.com;example.com",
			);
			expect(expanded).toContain("microsoft.com");
		...
		it("handles mixed synonym and non-synonym domains", () => {
			const expanded = domainSynonymsService.expandUrlFilterWithSynonyms(
				"microsoft.com example.com google.com",
			);
			expect(expanded).toContain("microsoft.com");
			expect(expanded).toContain("live.com");
			expect(expanded).toContain("outlook.com");
		...
		it("handles custom synonym groups", () => {
			setMockCustomSynonyms([["mycompany.com", "mycompany.net"]]);
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("mycompany.com");
			expect(expanded).toContain("mycompany.com");
			expect(expanded).toContain("mycompany.net");
		});

		it("case-insensitive expansion", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("MICROSOFT.COM");
		...
		it("expands subdomain with wildcard synonym group (eBay case)", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("signin.ebay.de");
		...
		it("base domain should NOT match wildcard pattern (only direct subdomains)", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("ebay.de");
		...
		it("expands wildcard entry with wildcard synonym group", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("*.ebay.de");
			expect(expanded).toContain("*.ebay.de");
			expect(expanded).toContain("*.ebay.com");
		...
		it("multi-level subdomain should NOT match wildcard pattern (only direct subdomains)", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("www.signin.ebay.de");
		...
		it("does not expand subdomain with specific port", () => {
			const expanded = domainSynonymsService.expandUrlFilterWithSynonyms(
				"signin.ebay.de:8080",
			);
		...
		it("does not expand wildcard subdomain with specific port", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("*.ebay.de:443");
		...
		it("handles Apple subdomain matching", () => {
			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("signin.apple.com");
	...
	describe("server synonyms", () => {
		it("uses server synonyms correctly", () => {
			setMockServerSynonyms([["serverdomain1.com", "serverdomain2.com"]]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("serverdomain1.com");
		...
		it("handles empty array", () => {
			...
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("anydomain.com");
			expect(synonyms).toEqual([]);
		});
		...
		it("allows multiple server groups", () => {
			setMockServerSynonyms([
				["servergroup1a.com", "servergroup1b.com"],
				["servergroup2a.com", "servergroup2b.com"],
			]);
			const synonyms1 =
				domainSynonymsService.getSynonymsForDomain("servergroup1a.com");
			expect(synonyms1).toContain("servergroup1b.com");
			expect(synonyms1).not.toContain("servergroup2a.com");

			const synonyms2 =
				domainSynonymsService.getSynonymsForDomain("servergroup2a.com");
			expect(synonyms2).toContain("servergroup2b.com");
			expect(synonyms2).not.toContain("servergroup1a.com");
		});

		it("server synonyms work alongside hardcoded synonyms", () => {
			setMockServerSynonyms([["servercompany.com", "servercompany.net"]]);
			const serverSynonyms =
				domainSynonymsService.getSynonymsForDomain("servercompany.com");
		...
		it("getServerSynonyms returns server-provided groups", () => {
			const testGroups = [
	...
	describe("custom synonyms", () => {
		it("uses custom synonyms correctly", () => {
			setMockCustomSynonyms([["domain1.com", "domain2.com"]]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("domain1.com");
		...
		it("handles empty array", () => {
			setMockCustomSynonyms([]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("anydo main.com");
			expect(synonyms).toEqual([]);
		});
		...
		it("allows multiple custom groups", () => {
			setMockCustomSynonyms([
				["group1a.com", "group1b.com"],
				["group2a.com", "group2b.com"],
			]);
			const synonyms1 =
				domainSynonymsService.getSynonymsForDomain("group1a.com");
			expect(synonyms1).toContain("group1b.com");
			expect(synonyms1).not.toContain("group2a.com");

			const synonyms2 =
				domainSynonymsService.getSynonymsForDomain("group2a.com");
			expect(synonyms2).toContain("group2b.com");
			expect(synonyms2).not.toContain("group1a.com");
		});
	});

	describe("Wildcard pattern matching behavior", () => {
		it("*.blub.com should match direct subdomain sub.blub.com", () => {
			setMockCustomSynonyms([["*.blub.com", "test.com"]]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("sub.blub.com");
			expect(synonyms).toContain("test.com");
		});

		it("*.blub.com should NOT match base domain blub.com", () => {
			setMockCustomSynonyms([["*.blub.com", "test.com"]]);
		...
		it("*.blub.com should NOT match nested subdomain sub.sub.blub.com", () => {
			setMockCustomSynonyms([["*.blub.com", "test.com"]]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("sub.sub.blub.com");
			expect(synonyms).not.toContain("test.com");
			expect(synonyms).toEqual([]);
		});

		it("*.blub.com should match multiple direct subdomains", () => {
			setMockCustomSynonyms([["*.blub.com", "test.com"]]);
			const synonyms1 =
				domainSynonymsService.getSynonymsForDomain("login.blub.com");
			expect(synonyms1).toContain("test.com");

			const synonyms2 =
				domainSynonymsService.getSynonymsForDomain("api.blub.com");
			expect(synonyms2).toContain("test.com");

			const synonyms3 =
				domainSynonymsService.getSynonymsForDomain("www.blub.com");
			expect(synonyms3).toContain("test.com");
		});
	});

	describe("Merging synonym groups", () => {
		it("merges server and custom synonyms when they overlap", () => {
			setMockServerSynonyms([["overlap.com", "serversynonym.com"]]);
			setMockCustomSynonyms([["overlap.com", "customsynonym.com"]]);
			const synonyms =
				domainSynonymsService.getSynonymsForDomain("overlap.com");
			expect(synonyms).toContain("customsynonym.com");
			expect(synonyms).toContain("serversynonym.com");
		});

		it("merged groups work bidirectionally", () => {
			setMockServerSynonyms([["companyA.com", "companyA.net"]]);
			setMockCustomSynonyms([["companyA.com", "companyA.org"]]);

			const synonymsFromCom =
				domainSynonymsService.getSynonymsForDomain("companyA.com");
			expect(synonymsFromCom).toContain("companya.net");
			expect(synonymsFromCom).toContain("companya.org");

			const synonymsFromNet =
				domainSynonymsService.getSynonymsForDomain("companyA.net");
			expect(synonymsFromNet).toContain("companya.com");
			expect(synonymsFromNet).toContain("companya.org");

			const synonymsFromOrg =
				domainSynonymsService.getSynonymsForDomain("companyA.org");
			expect(synonymsFromOrg).toContain("companya.com");
			expect(synonymsFromOrg).toContain("companya.net");
		});

		it("expands with merged synonym groups", () => {
			setMockServerSynonyms([["testdomain.com", "testdomain.net"]]);
			setMockCustomSynonyms([["testdomain.com", "testdomain.org"]]);

			const expanded =
				domainSynonymsService.expandUrlFilterWithSynonyms("testdomain.com");
