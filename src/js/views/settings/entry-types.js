...
const SettingsEntryTypesView = (props) => {
	...
	const [showWebsitePassword, setShowWebsitePassword] = useState(
		settingsDatastore.showWebsitePassword,
	);
	const [showApplicationPassword, setShowApplicationPassword] = useState(
		settingsDatastore.showApplicationPassword,
	);
	const [showTOTPAuthenticator, setShowTOTPAuthenticator] = useState(
		settingsDatastore.showTOTPAuthenticator,
	);
	...
	const [showEnvironmentVariables, setShowEnvironmentVariables] = useState(
		settingsDatastore.showEnvironmentVariables,
	);
	...
	const [showIdentity, setShowIdentity] = useState(
		settingsDatastore.showIdentity,
	);
	const [showElsterCertificate, setShowElsterCertificate] = useState(
		settingsDatastore.showElsterCertificate,
	);
	...
	return (
		<Grid container>
			...
			{entryTypes
				...
				.map((entryType) => (
					<Grid item xs={12} sm={12} md={12} key={entryType.value}>
						<Checkbox
							tabIndex={1}
